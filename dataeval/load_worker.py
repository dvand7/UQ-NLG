...
def _compare_generated_text_to_answers(pred_txt, reference_answers):
    ...
    sequence_dict = {_: 0.0 for _ in ["exact_match"] + rouge_types}
    ...
    for answer in unique_reference_answers:
        ...
        sequence_dict["exact_match"] = max(
            results["exact_match"], sequence_dict["exact_match"]
        )
        rouge_results = rouge.compute(predictions=predictions, references=references)
        for rouge_type in rouge_types:
            sequence_dict[rouge_type] = max(
                rouge_results[rouge_type], sequence_dict[rouge_type]
            )
...
def _compare_generated_texts_to_answers(preds, reference_answers):
    ...
    results = {
        _: _compare_generated_text_to_answers(_, reference_answers)
        for _ in pred_map.values()
    }
...
def _clean_sample(sample, tokenizer):
    ...
    def _clean_answer(old_text: str, old_token_ids, tokenizer, cot_used=False):
    ...
    if tokenizer is None:
        ...
        for i, generation in enumerate(sample["generations_ids"]):
            ...
            ret["generations"][i] = _clean_answer(
                sample["generations"][i], generation, tokenizer
            )
    ret["generations"] = {
...
def _get_semantic_similarities_sample(
    sample, judge_model: sc.ClassifyWrapper, clean=False, logger=None
):
    ...
    for i, ans_i in enumerate(unique_ans):
        for j, ans_j in enumerate(unique_ans[i + 1 :], i + 1):
            ...
            # original logic
            deberta_prediction = torch.stack([sim_mat[i, j], sim_mat[j, i]], 0).argmax(
                1
            )
            _log_fn(
                f"Q: {question} || A1: {ans_i} || A2: {ans_j} || {deberta_prediction}"
            )
            if deberta_prediction.min() == 0:
                has_semantically_different_answers = True
            else:
                semantic_set_ids[ans_j] = semantic_set_ids[ans_i]
            old_deberta_predictions.append(
                [question, ans_i, ans_j, deberta_prediction.min().item()]
            )
    return (
        dict(
            id=sample["id"],
            ...
            old={
                ...
                "syntactic_similarities": _old_syntactic_similarities(
                    sample["generations"][text_key]
                ),
            },
        ),
        old_deberta_predictions,
...
def _get_semantic_similarities(
    samples, judge_model: sc.ClassifyWrapper, clean=False, logger=None
):
    utils.seed_everything(10)
    result_dict, deberta_predictions = {}, []
    for sample in tqdm.tqdm(samples):
        result_dict[sample["id"]], deberta_predictions_ = (
            _get_semantic_similarities_sample(sample, judge_model, clean, logger)
        )
        deberta_predictions.extend(deberta_predictions_)
    return result_dict, pd.DataFrame(
        deberta_predictions, columns=["question", "ans1", "ans2", "deberta_prediction"]
    )
...
The possible answer is: ("""
    ...
    return dict(
        id=row["id"],
        logits=pd.DataFrame(
            torch.stack([ret[_] for _ in anss]).cpu().numpy(), columns=["True", "False"]
        ),
    )
...
Rating:"""
    return models.openai_query(
        prompt, model="gpt-3.5-turbo", attemptd_id=0, max_tries=50
    )


def _get_gpt_eval(
    samples, clean: bool, ith: int, dataset: str, logger=None, parallel=False
):
    text_key = "text_cleaned" if clean else "text"
    df = pd.DataFrame(
        {
...
def _get_rouge_parallel(samples, clean: bool, logger=None):
    ...
    df = pd.DataFrame(
        {
            key: [sample[key] for sample in samples]
            for key in ["id", "answer", "additional_answers"]
        }
    )
    df["text_key"] = text_key
...
def _get_lexical_similarities_sample(sample):
    ...
    for i, ans_i in enumerate(unique_ans):
        for j, ans_j in enumerate(unique_ans[i + 1 :], i + 1):
            sim_mat[i, j] = sim_mat[j, i] = rouge.compute(
                predictions=[ans_i], references=[ans_j], rouge_types=["rougeL"]
            )["rougeL"]
